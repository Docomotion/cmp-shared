...
namespace Docomotion.Shared.NumberUtils.Humanizer.Localisation.NumberToWords
{
    internal class EnglishNumberToWordsConverter : GenderlessNumberToWordsConverter
    {
        ...
        private const string AndSymbol = "and";
        ...
        private string Convert(long number, bool isOrdinal, bool addAnd = true)
        {
            ...
            if ((number / 1000) > 0)
            {
                var thousandPart = string.Format("{0} thousand", Convert(number / 1000));
                parts.Add(parts.Count > 0 && addAnd ? $"{AndSymbol} {thousandPart}" : thousandPart);
                number %= 1000;
            }

            if ((number / 100) > 0)
            {
                var hundredPart = string.Format("{0} hundred", Convert(number / 100));
                parts.Add(parts.Count > 0 && addAnd ? $"{AndSymbol} {hundredPart}" : hundredPart);
                number %= 100;
            }
